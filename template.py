...
class explanatoryTools(Scene):
    ...
    def add_column_form(self, *args):
        ...
        add_table, attention, max_col = column_form(upper=list(args), lower=[result], sign='+')
    ...
    def sub_column_form(self, *args):
        ...
        sub_table, attention, max_col = column_form(upper=list(args), lower=[result], sign='-')
    ...
    def mul_column_form(self, *args):
        result = args[0]
        for i in args[1:]:
            result = result*i
        sub_table = column_form(upper=list(args), lower=[result], sign='-')
        self.play(Create(sub_table))
        self.wait(1)
    
    def ask(self, *args, sign=''):
        sign = f' {sign} '
        question_text = Text(f'{sign.join(list(map(str, args)))} = ?')

        x_scale = min(config.frame_width-2, question_text.width)/question_text.width
        question_text.scale(x_scale)
        question_text.to_corner(UP*0.5)
    ...
    def add_line(self, *args, total_length=7):
        line_group = [NumberLine(x_range=[0, num, num], color=random_color_set(), stroke_width=4, length=total_length*num/sum(args)) for num in args]
        label_group = [Text(f'{num}').scale(0.5) for num in args]
        for i in range(len(args)):
            if i == 0 : 
                line_group[i].move_to((LEFT*(total_length/2)+UP*2.3),aligned_edge=LEFT)
                label_group[i].next_to(line_group[i], UP)
                self.play(Create(line_group[i], lag_ratio=0.2), Write(label_group[i]))
            else : 
                line_group[i].move_to(line_group[i-1].get_right(), aligned_edge=LEFT)
                label_group[i].next_to(line_group[i], UP)
                self.play(Create(line_group[i], lag_ratio=0.2), Write(label_group[i]))
        
        sum_line = NumberLine(length=total_length).move_to((line_group[0].get_left()),aligned_edge=LEFT)
        sum_label= BraceLabel(sum_line, '?', DOWN)
  

        self.play(Write(sum_label,lag_ratio=0.05))
    
    def sub_line(self, *args, total_length=7):
        line_group = [NumberLine(x_range=[0, num, num], color=random_color_set(), stroke_width=4, length=total_length*num/args[0]) for num in args]
        label_group = [Text(f'{num}').scale(0.5) for num in args]

        for i in range(len(line_group)):
            if i == 0: 
                line_group[i].move_to((LEFT*(total_length/2)+UP*2.3),aligned_edge=LEFT)
                label_group[i].next_to(line_group[i], UP*0.5)
                self.play(Create(line_group[i], lag_ratio=0.2), Write(label_group[i]))
            elif i == 1: 
                line_group[i].move_to((LEFT*(total_length/2)+UP*2.0), aligned_edge=LEFT)
        ...
        diff_line = DashedLine([0,0,0],[total_length-total_length*sum(args[1:])/args[0],0,0], color=WHITE).move_to((line_group[-1].get_right()),aligned_edge=LEFT)
        diff_label= BraceLabel(diff_line, '?', DOWN)

        self.play(Create(diff_line, lag_ratio=0.2))
